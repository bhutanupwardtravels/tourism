...
export function DataTable<TData, TValue>({
    ...
    const [columnFilters, setColumnFilters] = React.useState<ColumnFiltersState>(() => {
        ...
        if (category) {
            filters.push({ id: "category", value: category.split(",") });
        }
    ...
    const onColumnFiltersChange: OnChangeFn<ColumnFiltersState> = React.useCallback(
        (updaterOrValue) => {
            ...
            // Clear existing category/status filters from params first
            params.delete("category");
            params.delete("status");
            params.delete("title");

            newFilters.forEach(filter => {
                if (filter.id === "category" || filter.id === "status" || filter.id === "title") {

"use client";

import { PaginationState } from "@tanstack/react-table";
import { columns } from "./columns";
import { DataTableToolbar } from "./data-table-toolbar";
import { TestimonialCard } from "./testimonial-card";
import { DataTableWithView } from "@/components/admin/data-table/data-table-with-view";
import { DataTableFilterParam } from "@/components/admin/data-table/data-table";
import { Testimonial } from "../schema";

const filterParams: DataTableFilterParam[] = [{ id: "name" }];

interface TestimonialsTableProps {
    data: Testimonial[];
    pageCount: number;
    pagination: PaginationState;
}

export function TestimonialsTable({ data, pageCount, pagination }: TestimonialsTableProps) {
    return (
        <DataTableWithView
            storageKey="testimonials_view_preference"
            data={data}
            columns={columns}
            pageCount={pageCount}
            pagination={pagination}
            filterParams={filterParams}
            toolbar={DataTableToolbar}
            gridClassName="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6"
            renderCard={(row, { isMobile }) => (
                <TestimonialCard testimonial={row.original} showActionsOnClick={isMobile} />
            )}
        />
    );
}

...
export function DeleteCampaignDialog({ campaign, open, onOpenChange }: DeleteCampaignDialogProps) {
    ...
    const handleDelete = async () => {
        setIsDeleting(true);
        try {
            const result = await deleteCampaignAction((campaign.id || campaign._id)!);
            if (result.success) {
        ...
        <AlertDialog open={open} onOpenChange={onOpenChange}>
            <AlertDialogContent className="rounded-none">
                <AlertDialogHeader>
                    ...
                    <AlertDialogDescription>
                        This permanently deletes the campaign{" "}
                        <strong className="text-amber-600">{campaign.name}</strong>. Codes already
                        issued from it are kept, but they stop working once the campaign is gone.
                    </AlertDialogDescription>
                </AlertDialogHeader>
                <AlertDialogFooter>
                    <AlertDialogCancel
